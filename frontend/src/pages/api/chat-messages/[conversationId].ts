...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log('Chat messages proxy: Forwarding request to:', `${baseUrl}${apiPath}/${conversationId}/messages`);
    console.log('Chat messages proxy: Headers being sent:', {
    ...
    const backendRes = await axios.get(
      `${baseUrl}${apiPath}/${conversationId}/messages`,
      {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token || '',
    ...
    console.error('Chat messages proxy error details:');
    console.error('- Original error:', error.message);
    console.error('- Error code:', error.code);
    console.error('- Error response:', error.response?.data);
    console.error('- Error status:', error.response?.status);
    console.error('- Requested URL:', `${baseUrl}${apiPath}/${conversationId}/messages`);
